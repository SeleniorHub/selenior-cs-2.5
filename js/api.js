...
function rowToClient(r){return{id:String(r[0]),nome:r[1]||'',nicho:r[2]||'',fase:r[3]||'Onboarding',churn:r[4]||'baixo',dataInicio:r[5]||'',mrr:String(r[6]||'0'),indicador:r[7]||'',comissaoVal:r[8]||'',comissaoTipo:r[9]||'pct',checkpoints:r[10]?String(r[10]).split('||').filter(Boolean):[],done:r[11]?String(r[11]).split('||').filter(Boolean):[],nota:r[12]||'',depoimento:r[13]||'',status:r[14]||'ativo',dataFim:r[15]||''};}
